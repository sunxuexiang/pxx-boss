    ...
    return (
      <div>
        <GreyBox>
          ...
          <p>所属仓库：{goods ? goods.get('wareName') : '-'}</p>
        </GreyBox>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          >
            ...
            <Column
              ...
              key="vipPrice"
              render={(vipPrice) => {
                return vipPrice == null ? 0.0 : vipPrice.toFixed(2);
              }}
            />
            <Column
              title="库存"
              dataIndex="goodsWareStocks"
              key="goodsWareStocks"
              render={(goodsWareStocks) => {
                return goodsWareStocks
                  ? goodsWareStocks.length == 0
                    ? '暂无数据'
                    : goodsWareStocks.map((item) => {
                        return (
                          <div>
                            <span>
                              {item.wareName}：{item.stock}
                            </span>{' '}
                            &nbsp;&nbsp;
                          </div>
                        );
