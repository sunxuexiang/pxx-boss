...
export default class PicModal extends React.Component<any, any> {
  props: {
    relaxProps?: {
      ...
      cancelImgChoose: Function;
      activeKey: string;
      chooseRuleImg: Function;
      chooseImgs: any;
    };
  };
        ...
        title={
          <div style={styles.title}>
            ...
            <span style={styles.grey}>
              ...
              最多可选<strong style={styles.dark}>5</strong>张
            </span>
          </div>
        }
        zIndex={1000}
                    ...
                    return (
                      <div style={styles.navItem} key={k}>
                        <div style={styles.boxItem}>
                          <Checkbox
                            className="big-check"
                            checked={v.get('ruleChecked')}
                            onChange={(e) => this._chooseRuleImg(e, v)}
                          />
                          <img
                            src={v.get('artworkUrl')}
                            alt=""
   ...
   */
  _chooseImg = (e, v) => {
    const { chooseImg, choosedImgCount } = this.props.relaxProps;
    const checked = (e.target as any).checked;
    chooseImg({ check: checked, img: v, chooseCount: choosedImgCount });
  };

  _chooseRuleImg = (e, v) => {
    const { chooseRuleImg, choosedImgCount } = this.props.relaxProps;
    const checked = (e.target as any).checked;
    chooseRuleImg({
      ruleChecked: checked,
      img: v,
      chooseCount: choosedImgCount
    });
  };
   ...
   */
  _handleUploadClick = () => {
    const { cateId } = this.props.relaxProps;
    if (!cateId) {
