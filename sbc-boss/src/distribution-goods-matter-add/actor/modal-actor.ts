...
export default class ModalActor extends Actor {
  ...
  @Action('modal: toggleGoodsModal')
  toggleGoodsModal(state){
    return state.set('goodsModalVisible',!state.get('goodsModalVisible'))
  }

  @Action('modal: saveSkuIds')
  saveSkuIds(state,skuIds){
    return state.set('chooseSkuIds',fromJS(skuIds))
  }
  
  @Action('modal: saveRows')
  saveRows(state,rows){
    return state.set('chooseGoodsInfos',fromJS(rows))
  }

  @Action('chooseGoodsInfos :deleteSku')
  deleteSku(state,skuId){
    return state.set('chooseGoodsInfos',
    state.get('chooseGoodsInfos').filter(f=>f.get('goodsInfoId')!=skuId))
    .set('chooseSkuIds',fromJS([]))
  }
}
