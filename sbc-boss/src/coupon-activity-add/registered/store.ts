 ...
 */
  ...
  editInit = async (activityId) => {
    ...
    if (res.code == Const.SUCCESS_CODE) {
      ...
      activity.imageUrl = couponActivity.imageUrl;
  ...
  save = async () => {
    ...
    let imageList = this.state()
      .get('images')
      .toJS();
    // console.log(imageList[0].response[0],'imageList');
    // return
    // 2.格式化数据
    let params = {} as any;
    params.imageUrl = imageList[0] ? imageList[0].response[0] : '';
