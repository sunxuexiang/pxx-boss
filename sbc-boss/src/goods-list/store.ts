...
export default class AppStore extends Store {
  ...
  init = async (
    ...
    this.transaction(() => {
      if (cates.res.code === Const.SUCCESS_CODE) {
        this.dispatch('cateActor: init', fromJS(cates.res.context));
      }
      if (brands.res.code === Const.SUCCESS_CODE) {
  ...
  onSearch = async () => {
    ...
    // // const setSeachData = localStorage.setItem('SeachData', pageNum)
    // // console.log(setSeachData,'66666',localStorage);
  ...
  onPageSearch = async () => {
    let request: any = {
      ...
      manySpecs: this.state().get('manySpecs')
        ...
        : '0',
      wareId: this.state().get('wareId') || '0',
      storeName: this.state().get('storeName')
    };
    ...
    this.dispatch('form:field', { key: 'searchForm', value: request });
  ...
  _changeData = (data, list) => {
    let content = data.getIn(['goodsPages', 'content']);
    ...
    data = data.setIn(['goodsPages', 'content'], content);
