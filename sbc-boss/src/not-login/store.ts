...
export default class AppStore extends Store {
  ...
  init = async (activityId) => {
    ...
    let levelList = levRes.res.context.customerLevelVOList;
    console.log(activityId, 'activityIdactivityId');
    ...
    if (activityId) {
      ...
      if (res.code == Const.SUCCESS_CODE) {
        ...
        activity.longNotOrder = couponActivity.longNotOrder;
  ...
  save = async (joinLevel) => {
    ...
    params.longNotOrder = activity.longNotOrder;
    params.startTime = activity.startTime;
    params.endTime = activity.endTime;
    params.couponActivityType = 12; // 指定赠券
