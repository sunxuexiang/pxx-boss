...
export default class SpecifyAddForm extends React.Component<any, any> {
  ...
  render() {
    ...
    console.log(activity.toJS(),'789789');
    ...
    return (
      <NumBox>
        <Form style={{ marginTop: 20 }}>
          ...
          <FormItem {...formItemLayout} label="距上次支付多久可领劵(天)">
            {getFieldDecorator('longNotOrder', {
              rules: [
                {
                  required: true,
                  message: '距上次支付多久可领劵'
                ...
                {
                  validator: (rule, value, callback) => {
                    QMMethod.validatorEmoji(rule, value, callback, '距上次支付多久可领劵');
                  }
                }
              ],
              onChange: (e) => {
                store.changeFormField({ longNotOrder: e.target.value });
              },
              initialValue: activity.get('longNotOrder')
            })(
              <Input
                placeholder="输入数字（天）"
                style={{ width: 360 }}
