...
const importGoods = (ext) => {
  return Fetch('/pointsgoods/import/' + ext, {
    method: 'GET'
  });
};
...
export default class GoodsImport extends React.Component<any, any> {
        ...
        <div className="container">
          ...
          <Alert
            ...
            description={
              <ul>
                <li>
                  1、请先下载积分商品导入模板，按照批注中的要求填写数据，未按要求填写将会导致导入失败。
                </li>
                <li>
                  2、请选择 .xlsx或 .xls文件，文件大小≤2M，每次只能导入一个文件，建议每次导入不超过500条会员数据。
                </li>
                <li>
            ...
            <Spin spinning={this.state.loading}>
              <div className="steps-content" style={styles.center}>
                <Dragger
                  ...
                  accept=".xls,.xlsx"
                  headers={header}
                  action={Const.HOST + '/pointsgoods/excel/upload'}
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref = Const.HOST + `/pointsgoods/excel/template/${encrypted}`;
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref = Const.HOST + `/pointsgoods/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
