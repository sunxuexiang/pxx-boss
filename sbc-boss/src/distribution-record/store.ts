...
export default class AppStore extends Store {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (token) {
          ...
          // 新窗口下载
          const exportHref =
            Const.HOST + `/distribution/record/export/params/${encrypted}`;
  ...
  searchStoreInfos = async (value) => {
    if (value != undefined) {
      ...
      if (
        ...
        const { res } = await webapi.filterCompanyData({
          companyCode: value,
          pageNum: 0,
          pageSize: 5
        });
        if (res.code == Const.SUCCESS_CODE) {
          this.dispatch(
            'distribution:record:filterStore',
            res.context.storeSimpleInfos
          );
        }
      } else {
        //根据店铺的名称
        const { res } = await webapi.filterStoreData({
          storeName: value,
          pageNum: 0,
          pageSize: 5
        });
   ...
   * 根据选中的分销员信息过滤出其distributionId,并存放
   */
  saveStoreInfoFilter = (value) => {
    const filterStoreData = this.state().get('filterStoreData');
    const storeId = filterStoreData
      .filter((v) => v.get('value') == value)
      .get(0)
      .get('key');
    //存放最终检错所需的参数
