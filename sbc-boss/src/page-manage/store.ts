...
export default class AppStore extends Store {
  ...
  init = async ({ pageNum, pageSize } = { pageNum: 0, pageSize: 10 }) => {
    ...
    this.state()
      ...
      console.log(includePageTypeList,'includePageTypeListincludePageTypeListincludePageTypeList123');
  ...
  onhandleChange=(key,value)=>{
    this.dispatch('start:set',{key,value});
    if(key=='wareId') this.init();
  };
  ...
  onClickMenu = (key: string) => {
    ...
    console.log(includePageTypeList,'includePageTypeListincludePageTypeListincludePageTypeList');
  ...
  saveMagicPage = async (htmlStr) => {
    const { res } = await magicPageSave(htmlStr);
