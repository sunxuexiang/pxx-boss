...
export default class AppStore extends Store {
  ...
  init = async (liveRoomId) => {
    ...
    let LivePeopleList = this.state().get('LivePeopleList').toJS();
    let LivePraiseList = this.state().get('LivePraiseList').toJS();
   ...
   */
  onPeopleSave = async () => {
    let { currentLiveListTab, LivePeopleList, detail, peopleValue } =
      this.state().toJS();
   ...
   */
  onPraiseSave = async () => {
    let { currentLiveListTab, LivePraiseList, detail, praiseValue } =
      this.state().toJS();
    let { res } = await webApi.saveRule({
      type: currentLiveListTab,
      liveRoomId: detail.liveRoomId,
  ...
  changeLiveListTab = (value) => {
    ...
    let { liveRoomId } = this.state().get('detail').toJS();
