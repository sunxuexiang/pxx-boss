...
export default class FormActor extends Actor {
  defaultState() {
    return {
      settings: {
        ...
        doorPickContent: '',
        // 配送到店文案
        deliveryToStoreContent: '',
        // 同城配送文案
        intraCityLogisticsContent: '',
        // 指定物流文案
        specifyLogisticsContent: '',
        // 快递到家（到付）
        expressArrivedContent: ''
      },
      ...
      regEditor: null,
      // 快递到家(自费)
      regLogisticsContent: null,
      // 托运部
      expensesCostContent: null,
      // 同城配送
      regExpressContent: null,
      // 自提
      regPickSelfContent: null,
      // 配送到店
      refDeliveryToStore: null,
      // 快递到家(到付)
      refExpressToHomePay: null
    };
  }

  @Action('setting:activeTab')
  ...
  init(state: IMap, setting) {
    ...
    return state.set('settings', setting);
  }
  ...
  @Action('setting:refDeliveryToStore')
  refDeliveryToStore(state, regEditor) {
    return state.set('refDeliveryToStore', regEditor);
  }

  @Action('setting:refExpressToHomePay')
  refExpressToHomePay(state, regEditor) {
    return state.set('refExpressToHomePay', regEditor);
  }
}
