...
@Relax
export default class SettingSupForm extends React.Component<any, any> {
  props: {
    relaxProps?: {
      supplierBanner: string;
      setSupplierBanner: Function;
      onSubmit: Function;
    };
  };
  ...
  static relaxProps = {
    ...
    setSupplierBanner: noop,
    onSubmit: noop
  };

  render() {
    const { supplierBanner } = this.props.relaxProps;

    const supplierBannerImage = supplierBanner
      ? JSON.parse(supplierBanner)
      : [];
    return (
      <div>
        <Form>
          <Row>
            <Col span={24}>
              ...
              >
                  ...
                  <Col span={24}>
                    <div className="clearfix bannerImg">
                      <QMUpload
                        ...
                        name="uploadFile"
                        onChange={this._editSupplierBanners}
                        fileList={supplierBannerImage}
                        accept={'.jpg,.jpeg,.png,.gif'}
                        beforeUpload={this._checkUploadFile.bind(this, 2)}
                      >
                        {supplierBannerImage.length >= 5 ? null : (
                          <div>
                ...
                <Tips title="商家登录页广告图，最多可添加5张，图片格式仅限jpg、jpeg、png、gif，建议尺寸1920*550px，大小不超过2M" />
              </FormItem>
            </Col>
          </Row>
   ...
   * @private
   */
  _editSupplierBanners = ({ file, fileList }) => {
    const { setSupplierBanner } = this.props.relaxProps;
    //当所有图片都被删除时
    ...
    setSupplierBanner(JSON.stringify(fileList));
  };
