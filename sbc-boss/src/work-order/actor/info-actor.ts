...
export default class InfoActor extends Actor {
   ...
   * 设置勾选的id
   */
  @Action('info:setCustomerStatus')
  setCustomerStatus(state: IMap, ids) {
    return state.set('customerStatus', ids);
  }
   ...
   * 设置新增/编辑的表单信息
   */
  @Action('info:setMergeAccountDetails')
  mergeAccountDetails(state: IMap, data: IMap) {
    return state.set('mergeAccountDetails', data);
  }
  ...
  @Action('form:checkStatus')
  checkState(state: IMap, index: string) {
    return state.set('tabStatus', index);
  }
}
