  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Column title="供应商名称" key="storeName" dataIndex="storeName" />
  ...
  _menu = (settleId, settleStatus) => {
    return (
      <div className="operation-box" style={{width:160}}>
        {settleStatus == 0 &&
          checkAuth('m_billing_provider_details_2') && (
              <a onClick={() => history.push(`/billing-provider-details/${settleId}`)}>
                设为已结算
              </a>
          )}
        {settleStatus == 2 &&
        ...
        {checkAuth('m_billing_provider_details_1') && (
            <a onClick={() => history.push(`/billing-provider-details/${settleId}`)}>
              查询明细
            </a>
        )}
