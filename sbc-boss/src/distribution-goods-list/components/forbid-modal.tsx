...
export default class ForbidModal extends React.Component<any, any> {
  ...
  static relaxProps = {
    ...
    // 禁售单个商品的id
    forbidGoodsInfoId: 'forbidGoodsInfoId',
    // 禁售原因
  ...
  render() {
    const { modalVisible, refuseFlag } = this.props.relaxProps;
    ...
    return (
       <Modal  maskClosable={false}
        title={`请填写${refuseFlag == 0 ? '驳回' : '禁止分销'}原因`}
  ...
  _handleSubmit = () => {
    ...
    form.validateFields(null, (errs) => {
      if (!errs) {
        //提交
        const { onRefuseFunc, forbidGoodsInfoId } = this.props.relaxProps;
        onRefuseFunc(forbidGoodsInfoId);
      }
    });
  };
...
class ForbidModalForm extends React.Component<any, any> {
  ...
  render() {
    ...
    const { forbidReason, onFieldChange, refuseFlag } = this.props.relaxProps;
    const tipTxt = `${refuseFlag == 0 ? '驳回' : '禁止分销'}原因`;
    return (
