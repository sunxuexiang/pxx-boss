...
export default class CouponActor extends Actor {
  ...
  @Action('modalActor: change:coupons')
  changeCoupon(state: IMap, { couponId, field, value }) {
    return state.updateIn(['activity', 'coupons'], (rows) =>
      rows.map((coupon) => {
        if (coupon.get('couponId') == couponId) {
          return coupon.set(field, value);
        }
        return coupon;
      })
    );
