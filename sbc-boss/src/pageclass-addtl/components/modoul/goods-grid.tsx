...
export default class GoodsGrid extends React.Component<any, any> {
  ...
  init = async (params,limitNOSpecialPriceGoods,noLimitWare,marketingId) => {
    ...
    let wareId=this.state.searchForm.wareId||''
    const ress = await fetch(
      `${
        Const.X_XITE_OPEN_HOST
      }/api/page/list?includePageTypeList=[%22poster%22]&excludePageTypeList=[]&platform=weixin&pageNo=${params.pageNum}&pageSize=${params.pageSize}&wareId=${wareId}`,
      {
        method: 'GET',
        headers: {
