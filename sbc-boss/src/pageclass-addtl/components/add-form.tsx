...
export default class RegisteredAddForm extends React.Component<any, any> {
  ...
  componentWillReceiveProps(nextpops) {
    ...
    this.setState({
      selectedRows: fromJS(actvie.selectedRows), selectedRows1: fromJS(actvie.selectedRows1), goodsModal: {
  ...
  render() {
    ...
    const { skuExists, selectedRows, skuExists1, selectedRows1, statusa } = this.state;
    ...
    return (
      <NumBox>
        <Form style={{ marginTop: 20 }}>
          ...
          {
            isSuit == 0 ?
              <FormItem {...formItemLayout} label="左侧图跳转链接" required={true}>
                {getFieldDecorator(
                  'jumpLink',
                  {}
                )(
          ...
          {isSuit1 == 0 ?
            <FormItem {...formItemLayout} label="右侧图跳转链接" required={true}>
              {getFieldDecorator(
                'jumpLink1',
                {}
              )(
  ...
  skuSelectedBackFun = async (selectedSkuIds, selectedRows) => {
    ...
    if (selectedSkuIds.length > 0) {
      if (this.state.statusa == 1) {
        ...
        console.log(selectedRows.toJS()[0].title,'selectedRows.toJS()[0].title');
