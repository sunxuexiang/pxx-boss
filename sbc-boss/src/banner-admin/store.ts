...
export default class AppStore extends Store {
  ...
  queryPage = async ({ pageNum, pageSize } = { pageNum: 0, pageSize: 10 }) => {
    ...
    if (pageRes.code === Const.SUCCESS_CODE) {
      this.transaction(() => {
        ...
        // 设置分页数据
        this.dispatch('info:setPageData', pageRes.context.bannerAdminVOPage);
        // 设置当前页码
    ...
    await this.getOneCate();
  };

  //获取一级分类

  getOneCate = async () => {
    const { res: oneCates } = await webapi.getOneCate();
    if (oneCates.code === Const.SUCCESS_CODE) {
      this.dispatch('goodsCate:setGodsCateDate', oneCates.context);
    }
  };
  ...
  onChangeStatus = async (id, status) => {
    ...
    const { res: result } = await webapi.modifyStatus(param);
  ...
  onSave = async () => {
    ...
    let result;
    let shift = JSON.parse(formData.get('bannerImg')).shift();
    if (shift == null || shift.status == 'uploading') {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (token) {
          ...
          const exportHref = Const.HOST + `/banneradmin/export/${encrypted}`;
          // 新窗口下载
