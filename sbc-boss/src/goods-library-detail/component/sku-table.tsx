...
class SkuForm extends React.Component<any, any> {
  ...
  _getColumns = () => {
    ...
    columns = columns.push({
      title: (
        <div>
          ...
          <br />
          {/*<Checkbox*/}
          {/*checked={vipPriceChecked}*/}
          {/*onChange={(e) => this._checkVipPrice(e)}*/}
          {/*>*/}
          {/*全部相同&nbsp;*/}
          {/*<Tooltip*/}
          {/*placement="top"*/}
          {/*title={'勾选后所有SKU都使用相同的大客户价'}*/}
          {/*>*/}
          {/*<a style={{ fontSize: 14 }}>*/}
          {/*<Icon type="question-circle-o" />*/}
          {/*</a>*/}
    ...
    columns = columns.push({
      ...
      render: (rowInfo) =>
        specSingleFlag ? null : (
          <Button onClick={() => this._deleteGoodsInfo(rowInfo.id)}></Button>
        )
    });
  ...
  _editGoodsItem = (id: string, key: string, e: any) => {
    ...
    // 3.如果是门店价
    if (key == 'vipPrice' && vipPriceChecked) {
      // 3.1.同store中的大客户价
      synchMarketPrice();
      // 3.2.同步form中的大客户价
      const fieldsValue = this.props.form.getFieldsValue();
   ...
   * 勾选是否批量设置门店价
   */
  _checkVipPrice = async (e) => {
    const { updateVipPriceChecked, goodsList } = this.props.relaxProps;
    await updateVipPriceChecked(e.target.checked);
    const goodsInfo = goodsList.get(0);
    if (goodsInfo) {
      this._editGoodsItem(
        goodsInfo.get('id'),
        'vipPrice',
        goodsInfo.get('vipPrice')
      );
    }
  };
