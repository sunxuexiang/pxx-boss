...
export default class AppStore extends Store {
  ...
  init = async (goodsId?: string) => {
    if (goodsId) {
      ...
      // 保证品牌分类等信息先加载完
      await Promise.all([getBrandList(), getCateList()]).then((results) => {
        this.dispatch(
  ...
  saveAll = async () => {
    ...
    if (
      brandList &&
      !brandList.find((b) => b.get('brandId') == goods.get('brandId'))
    ) {
   ...
   * 更新是否批量设置大客户价
   */
  updateVipPriceChecked = async (checked?: boolean) => {
    this.dispatch('goodsSpecActor: updateVipPriceChecked', checked);
  };
