...
export default class List extends React.Component<any, any> {
        ...
        <Column
          ...
          render={(img) =>
            img.advertisingConfigList.map((item, index) => {
              return (
                item.advertisingImage ? <img src={item.advertisingImage} key={index}  style={styles.imgItem}  /> : ''
              )
            })
          }
        />
  ...
  FunisLink(record:any) {
    ...
    if (record.advertisingType == 0) {
      return `/pagehome-addtl/${form.get('wareId')}/${record.advertisingId}`
    } else if (record.advertisingType == 1) {
      return `/pageclass-addtl/${form.get('wareId')}/${record.advertisingId}`
    } else {
      return `/pagehome-swit/${form.get('wareId')}/${record.advertisingId}`
    }
  }
}
const styles = {
