...
export default class SearchList extends React.Component<any, any> {
                ...
                >
                  <thead className="ant-table-thead">
                    <tr>
                      ...
                      <th style={{ width: '17%' }}>商品</th>
                      <th style={{ width: '12%' }}> 囤货单号</th>
                      <th style={{ width: '11%' }}> 退单时间</th>
                      <th style={{ width: '10%' }}> 客户名称</th>
                      <th style={{ width: '13%' }}> 商家</th>
                      <th style={{ width: '10%' }}> 业务员</th>
                      <th style={{ width: '8%' }}> 支付方式</th>
                      <th style={{ width: '9%' }}> 应退金额</th>
                      <th style={{ width: '7%' }}> 退单状态</th>
                    </tr>
                  </thead>
          ...
          {total > 0 ? (
            <Pagination
              ...
              onShowSizeChange={(pageNum, pageSize) => {
                init({ pageNum: 0, pageSize: pageSize, flushSelected: false });
              }}
            />
          ) : null}
        </div>
  ...
  _renderProviderName(son) {
    if (son && son.getIn(['supplier', 'supplierName'])) {
      return (
        <span>
          供应商：{son && son.getIn(['supplier', 'supplierName'])}&nbsp;
            ...
            >
              <thead>
                <tr>
                  ...
                  >
                    ...
                    >
                      {this._renderProviderName(v.get('tradeVO'))}
                    </span>
                    <span style={{ marginRight: 0, float: 'right' }}>
                      {/* {(returnFlowState === 'RECEIVED' ||
                        ...
                        refundStatus === 3 && (
                          <AuthWrapper functionName="drodf002">
                            <a
                              ...
                              onClick={async () => {
                                onRefundModalChange({
                                  rid:v.get('id'),
                                  actualReturnPrice:v.getIn(['returnPrice', 'totalPrice']),
                                  balancePrice:v.getIn(['returnPrice', 'balancePrice'])||0,
                                })
                                // // 校验是否有退款权限, drodf002 为退款权限的功能编号
                                // const haveAuth = checkAuth('drodf002');
                                // if (haveAuth) {
                                //   if (payType == 1) {
                        ...
                        <AuthWrapper functionName="drodf002">
                          <a
                            ...
                            onClick={() => {
                              // onRefundModalChange({
                              //     rid:v.get('id'),
                              //     actualReturnPrice:v.getIn(['returnPrice', 'actualReturnPrice']),
                              //     balancePrice:v.getIn(['returnPrice', 'actualBalanceReturnPrice'])||0,
                              // });
                      ...
                      <AuthWrapper functionName={'drodf001'}>
                        <Link
                          style={{ marginLeft: 20, marginRight: 20 }}
                          to={`/th_order-return-detail/${rid}`}
                        >
                          查看详情
                        </Link>
                      </AuthWrapper>
              ...
              <tbody>
                <tr>
                  ...
                  {/*客户名称*/}
                  <td style={{ width: '10%' }}>{v.getIn(['buyer', 'name'])}</td>
                  <td
                    style={{
                      width: '13%'
                    }}
