...
export default class ChinaMap extends React.Component<ChinaMapProps, IState> {
  ...
  async componentDidMount() {
    ...
    //@ts-ignore
    this.chinaMap = window.echarts && window.echarts.init(document.getElementById('chinaMap'));

    this.chinaMap.on('click', this.onChartClick);

    //@ts-ignore
    this.provinceMap = window.echarts && window.echarts.init(
  ...
  _drewCharts = (param: IParam = { data: { code: 320000, name: '江苏' } }) => {
    if(this.provinceMap && this.chinaMap){
