...
export const th_refundOffline = (rid: string, params = {}) => {
...
export const th_refundOnline = (rid: string) => {
  return Fetch<TResult>(`/return/pile/refund/${rid}/online`, {
    method: 'POST'
  });
};
...
export const th_checkRefundStatus = (rid: string) => {
...
// 囤货
export function th_queryOfflineCustomerAccountByReturnId(rId: string) {
  return Fetch<TResult>(`/return/pile/customer/account/${rId}`);
}
...
export const refundOrderList = (params = {}) => {
  return Fetch<TResult>(`/account/refundOrderList`, {
    method: 'POST',
    body: JSON.stringify(params)
  });
};
