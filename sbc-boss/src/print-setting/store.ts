...
export default class AppStore extends Store {
  ...
  init = async () => {
    const { res } = (await webapi.fetchPrintSetting()) as any;
    if (res.code == Const.SUCCESS_CODE) {
      this.dispatch('printSetting:init', fromJS(res.context.printConfigVO));
    }
  };
   ...
   */
  refEditorHead = (head) => {
    this.dispatch('setting: regEditorHead', head);
  };

  /**
   * 将editor ref对象存储到actor中
   */
  refEditorBottom = (bottom) => {
    this.dispatch('setting: regEditorBottom', bottom);
  };
   ...
   */
  onSaveFunc = async () => {
    let printSetting = this.state().get('printSetting');

    let editorHead = this.state().get('regEditorHead');

    let editorBottom = this.state().get('regEditorBottom');
    if (!editorHead.getContent) {
      message.error('请输入打印模板的头部');
      return false;
    }
    if (!editorBottom.getContent) {
      message.error('请输入打印模板的尾部');
      return false;
    }
    printSetting.printHead = editorHead.getContent
      ? editorHead.getContent()
      : '';
    printSetting.printBottom = editorBottom.getContent
      ? editorBottom.getContent()
      : '';

    const { res } = (await webapi.modifyPrintSetting(printSetting)) as any;
    if (res.code == Const.SUCCESS_CODE) {
      await this.init();
      message.success('保存成功');
    } else {
