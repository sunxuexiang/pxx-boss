...
@Relax
export default class ReplaceAliModal extends React.Component<any, any> {
  _form;
  GateWaysForm: any;
  ...
  props: {
    relaxProps?: {
      replace_ali_visible: boolean;
      onSaveChannel: Function;
      onCancelReplaceAli: Function;
      channelJson: IMap;
  ...
  static relaxProps = {
    replace_ali_visible: 'replace_ali_visible',
    onSaveChannel: noop,
    onCancelReplaceAli: noop,
    channelJson: 'channelJson',
    onFormValueChange: noop
  };

  render() {
    const { replace_ali_visible, onCancelReplaceAli } = this.props.relaxProps;

    const GateWaysForm = this.GateWaysForm;

    if (!replace_ali_visible) {
      return null;
    }
    return (
      <Modal
          ...
          <Alert
            message={
              <ul>
                <li>风险提示</li>
                <li>
                  2017年11月13日，央行下发了《关于进一步加强无证经营支付业务整治工作的通知》(银办发[2017]217号文)，
                  文中对无证经营支付业务筛查要点、认定标准及持证机构违规情形的说明，是否开启余额支付功能，
                  请参考相应的政策法规，一旦开启，将承担相关法律责任。
                </li>
              </ul>
            }
            type="info"
