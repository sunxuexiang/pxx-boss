...
export default class ShopRuleActor extends Actor {
  defaultState() {
    return {
      ...
      // 表单内容
      formData: {
        // 是否使用上级积分获取比例 0: 否  1: 是
        isParentPointsRate: 0
      },
      // 上级积分获取比例
      parentRate: 0
    };
   ...
   * 修改表单内容
   */
  @Action('points-shop-rule: editFormData')
   ...
   * @param state
   */
  @Action('points-shop-rule: rate: use')
  useParentRateF(state: IMap, num: number) {
    return state.setIn(['formData', 'isParentPointsRate'], num);
  }
}
