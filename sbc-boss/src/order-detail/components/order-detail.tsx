...
import { Button, Col, Modal, Row, Table, Icon, Input, Tooltip } from 'antd';
import { Const, ImageModal, noop, history } from 'qmkit';
...
const columns = [
  ...
  },
  {
    ...
    render: (price, row) => (
      <span>
        ￥{(row.priceChanged === 1 ? row.levelPrice : price).toFixed(2)}
      </span>
    )
  },
  {
    ...
    render: (changedPrice, row, index) => (
      <span>￥{(changedPrice ? changedPrice : row.levelPrice).toFixed(2)}</span>
    )
  },
  {
    title: '库位',
  ...
  // },
  {
    ...
    render: (row) => {
      // .toFixed(2)
      let t = NP.times(row.num, row.price) / 1 || 0;
      let price = NP.minus(t / 1, row.splitPrice || 0);
      return (
        <span>
          ￥
          {row.walletSettlements && row.walletSettlements.length
            ? NP.minus(
                price,
                row.walletSettlements[0].reduceWalletPrice
              ).toFixed(2)
            : price
            ? price.toFixed(2)
            : '0.00'}
  ...
  },
  {
    ...
    render: (row) => (
      <span>
        ￥
        {/* {((row.num * row.levelPrice) - (row.num * row.price - row.splitPrice)).toFixed(2)} */}
        {row.walletSettlements && row.walletSettlements.length
          ? (
  ...
  },
  {
    ...
    render: (row) => (
      <span>
        {row.returnCoin || row.returnCoin === 0
          ? `￥${row.returnCoin.toFixed(2)}`
          : '-'}
      </span>
    )
  }
...
export default class OrderDetailTab extends React.Component<any, any> {
  ...
  render() {
    ...
    const tradePrice = detail.get('tradePrice').toJS() as {
      ...
      cmbDiscountsPrice: number; //商行优惠金额
      packingPrice: number; //纸箱费
      discountsPriceDetails: List<any>; //订单优惠金额明细
      paidPrice: number; //囤货支付
      balancePrice: number; //鲸币抵扣
      discountOfJingTie: number; //鲸币抵扣
    };
      ...
      <div>
        <div style={styles.headBox as any}>
          ...
          <Row>
            ...
            <Col span={8}>
              <p style={styles.darkText}>
                店铺名称：{detail.getIn(['supplier', 'storeName'])}
              </p>
              <p style={styles.darkText}>
            ...
            <Col span={8}>
              返鲸币：
              {detail.get('returnCoin') || '无'}
            </Col>
          </Row>

          {/* {this._renderBtnAction(detail.get('id'))} */}
        </div>
        ...
        >
          ...
          <label style={styles.inforItem}>
            发货仓：{detail.get('wareHouseNmae') || detail.get('wareHouseCode')}
          </label>
        </div>
      </div>
    );
  ...
  _renderSelfPickTime = (time) => {
    ...
    if (!moment(moment(time).format('HH:mm:ss')).isBefore(checkTime)) {
      pickTime = moment(time).add(1, 'd').format('YYYY年MM月DD日');
    }
