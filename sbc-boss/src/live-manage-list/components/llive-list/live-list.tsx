  ...
  _columns = [
    ...
    },
    {
      ...
      render: (time, rowInfo) => {
        return (
          <div>
            <div>{Moment(time).format(Const.TIME_FORMAT).toString() + '~'}</div>
            <div>
              {rowInfo.endTime
                ? Moment(rowInfo.endTime).format(Const.TIME_FORMAT).toString()
                : null}
            </div>
          </div>
        );
    ...
    },
    {
      ...
      title: '直播时长',
      render: (_, rowInfo) => {
        if (rowInfo.liveStatus === 2 && rowInfo.startTime && rowInfo.endTime) {
          const times = Moment(rowInfo.endTime).diff(
            Moment(rowInfo.startTime),
            'minutes'
          );
  ...
  _getOption = (rowInfo) => {
    ...
    return (
      <div>
        ...
        {rowInfo.liveStatus === 2 && rowInfo.mediaUrl && (
          <a
            style={styles.edit}
            onClick={() => {
              window.open(rowInfo.mediaUrl, '_blank');
            }}
          >
            直播回放
          </a>
        ...
        {/* {
          rowInfo.status!=1?
          <Popconfirm title="是否确认删除该直播间？" onConfirm={()=>onliveDel(rowInfo.liveId)} okText="确认" cancelText="取消">
            <a
              style={styles.edit}
