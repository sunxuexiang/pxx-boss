...
export default class RejectModal extends React.Component<any, any> {
  ...
  static relaxProps = {
    ...
    rejectDrawCashId: 'rejectDrawCashId',
  ...
  _handleOK = () => {
    this._form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        const { onAuditStatus, rejectDrawCashId } = this.props.relaxProps;
        onAuditStatus(rejectDrawCashId, values['rejectReason']);
        this._modalClose();
      }
    });
  };
