 ...
 */
const importLogisticsCompany = (ext, marketId, logisticsType) => {
  return Fetch(`/marketLogistics/import/${ext}/${marketId}/${logisticsType}`, {
    method: 'GET'
  });
};
...
export default class GoodsImport extends React.Component<any, any> {
    ...
    return (
      <div>
        ...
        <div className="container">
          <Headline title="物流设置导入" />
          <Alert
            message="操作说明："
            description={
              <ul>
                <li>
                  1、请先下载物流设置导入模板，按照批注中的要求填写数据，未按要求填写将会导致导入失败。
                </li>
                <li>
                  2、请选择 .xlsx或
            ...
            <Spin spinning={this.state.loading}>
              <div className="steps-content" style={styles.center}>
                <Dragger
                  ...
                  accept=".xls,.xlsx"
                  headers={header}
                  action={Const.HOST + '/marketLogistics/excel/upload'}
                ...
                <div>
                  <Button
                    type="primary"
                    onClick={() => this._importLogisticsCompany(0)}
                    disabled={isImport}
                  >
                    {`${errBtn ? '重新' : '确认'}导入托运部`}
                  </Button>
                  <Button
                    ...
                    onClick={() => this._importLogisticsCompany(1)}
                    disabled={isImport}
                  >
                    {`${errBtn ? '重新' : '确认'}导入指定专线`}
                  </Button>
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/marketLogistics/downloadTemplate/${encrypted}`;
  ...
  _importLogisticsCompany = async (logisticsType) => {
    ...
    const importRes: any = await importLogisticsCompany(
      ext,
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/marketLogistics/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
