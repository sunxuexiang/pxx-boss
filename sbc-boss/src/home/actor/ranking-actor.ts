...
export default class RankingActor extends Actor {
  defaultState() {
    return {
      ...
      storeStatisticRanking: [], //店铺流量排行TOP10
      storeTradeRanking: [] //店铺交易排行TOP10
    };
  }
   ...
   * 设置店铺流量排行TOP10
   */
  @Action('ranking-actor: setStoreStatisticRanking')
  setStoreStatisticRanking(state: IMap, res) {
    return state.set('storeStatisticRanking', fromJS(res));
  }

  /**
   * 设置店铺交易排行TOP10
   */
  @Action('ranking-actor: setStoreTradeRanking')
  setStoreTradeRanking(state: IMap, res) {
    return state.set('storeTradeRanking', fromJS(res));
  }
}
