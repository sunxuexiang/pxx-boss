...
import { Modal, Checkbox, Row, Input } from 'antd';
...
export default class CheckboxModal extends React.Component<any, any> {
   ...
   */
  _handleOk = () => {
    const { modalType, saveCheckbox, checkBoxDefaultVal } =
      this.props.relaxProps;
    const { checkedValues, changeFlag } = this.state;
    saveCheckbox(
      modalType,
      changeFlag ? checkedValues : checkBoxDefaultVal,
      () => {
        this.setState({
          checkedValues: [],
          filterWords: '',
  ...
  onCheckAllChange = (e) => {
    ...
    const { allMarkets, allStores, modalType, checkBoxDefaultVal } =
      this.props.relaxProps;
