...
export default class StepTwo extends React.Component<any, any> {
  ...
  render() {
    ...
    const businessLicence =
      info.get('businessLicence') && info.get('businessLicence') !== 'null'
        ? JSON.parse(info.get('businessLicence'))
        : [];
    const frontIDCard =
      info.get('frontIDCard') && info.get('frontIDCard') !== 'null'
        ? JSON.parse(info.get('frontIDCard'))
        : [];
    const backIDCard =
      info.get('backIDCard') && info.get('backIDCard') !== 'null'
            ...
            <Col span={12}>
              <FormItem {...formItemLayout} label="法人身份证">
                {getFieldDecorator('idCardNo', {
                  initialValue: info.get('idCardNo'),
                  rules: [
                    {
                      pattern: ValidConst.idCard,
                      message: '请输入正确的身份证号码'
                    }
                  ]
                })(
