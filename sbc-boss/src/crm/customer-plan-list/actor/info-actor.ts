...
export default class InfoActor extends Actor {
   ...
   */
  @Action('tab: change')
  changeTable(state: IMap, key: String) {
    return state.set('key', key).set('planStatus', key);
  }

  /**
   * 分群信息列表
   */
  @Action('info: setGroupInfoList')
  setGroupInfoList(state: IMap, groupInfoList) {
    return state.set('groupInfoList', groupInfoList);
  }
}
