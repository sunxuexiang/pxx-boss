...
import { Modal, Form, Input, Icon, message, InputNumber } from 'antd';
import { Relax } from 'plume2';
import { Map, fromJS } from 'immutable';
import { IMap } from 'typings/globalType';
import { noop, Tips, Const, QMUpload, ValidConst } from 'qmkit';
...
export default class BrandModal extends React.Component<any, any> {
  ...
  render() {
    ...
    return (
      <Modal
        maskClosable={false}
        title={formData.get('brandId') ? '编辑' : '增加'}
...
class BrandModalForm extends React.Component<any, any> {
  ...
  render() {
    const { maxNum } = this.props.relaxProps;
    const { brandName, nickName, logo, brandSeqNum, brandId } =
    ...
    const initNum = brandId ? brandSeqNum : maxNum + 1;
    return (
  ...
  _changeBrandSeqNum = (e) => {
    const { editFormData } = this.props.relaxProps;
    editFormData(Map({ brandSeqNum: e }));
  };
