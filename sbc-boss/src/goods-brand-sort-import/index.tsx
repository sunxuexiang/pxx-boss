 ...
 */
const importGoods = (ext, cateId) => {
  return Fetch('/brandSort/rel/import/' + ext + '/' + cateId, {
    method: 'GET'
  });
};
...
export default class GoodsImport extends React.Component<any, any> {
  ...
  render() {
    ...
    const { cateId, cateName } = this.props.location.state;
    return (
      <div>
        <BreadCrumb thirdLevel={true}>
          ...
          <Breadcrumb.Item>{'品牌排序导入'}</Breadcrumb.Item>
        </BreadCrumb>
        <div className="container">
          <Headline title="品牌排序导入" />
          <Alert
            message="操作说明："
            description={
              <ul>
                <li>
                  1、请先下载品牌排序导入模板，按照批注中的要求填写数据，未按要求填写将会导致导入失败。
                </li>
                <li>
                  2、请选择 .xlsx或
            ...
            <Spin spinning={this.state.loading}>
              <div className="steps-content" style={styles.center}>
                <Dragger
                  ...
                  accept=".xls,.xlsx"
                  headers={header}
                  action={Const.HOST + '/brandSort/rel/excel/upload'}
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref = Const.HOST + `/brandSort/rel/template/${encrypted}`;
  ...
  _importGoods = async () => {
    ...
    const importRes: any = await importGoods(ext, cateId);
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/brandSort/rel/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
