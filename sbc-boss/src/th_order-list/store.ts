...
export default class AppStore extends Store {
   ...
   * @param pageSize
   */
  init = ({ pageNum, pageSize } = { pageNum: 0, pageSize: this.state().get('pageSize')||10 }) => {
    ...
    webapi.fetchOrderList({ ...form, pageNum, pageSize }).then(({ res }) => {
      if (res.code == Const.SUCCESS_CODE) {
        this.transaction(() => {
          ...
          this.dispatch('list:page', fromJS({ currentPage: pageNum + 1,pageSize }));
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (token) {
          ...
          // 新窗口下载
          const exportHref =
            Const.HOST + `/newPileTrade/export/params/${encrypted}/${encryptedable}?isDetailed=${disabled}`;
