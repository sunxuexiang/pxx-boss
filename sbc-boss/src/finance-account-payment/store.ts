...
export default class AppStore extends Store {
  ...
  initOffLineAccounts = async () => {
    ...
    this.transaction(() => {
      ...
      this.dispatch('list:init', fromJS(res.context.payWithDrawDTOList));
    });
  };
  ...
  onEdit = (id: string) => {
    const account = this.state()
      .get('dataList')
      .find((account) => account.get('withdrawId') == id);
  ...
  onSave = async (params) => {
    if (this.state().get('edit')) {
      params.withdrawId = this.state().getIn(['accountForm', 'withdrawId']);
