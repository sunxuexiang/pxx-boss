...
export default class AppStore extends Store {
   ...
   */
  onShow = async () => {
    const { res } = await webapi.fetchInvoiceProjects();
    this.transaction(() => {
      this.dispatch('modal:show');
      this.dispatch('invoice:invoiceProject', res);
  ...
  onSearchByOrderNo = async (orderNo: string) => {
    ...
    const { res: fetchAddressInfos } = await webapi.fetchAddressInfos(
      orderInvoiceDetail.context.customerId
    );
    const { res: invoiceConfig } = await webapi.invoiceConfig();

    if (fetchAddressInfos.code == Const.SUCCESS_CODE) {
      this.transaction(() => {
        this.dispatch(
          'orderInvoice:orderInvoiceDetail',
          orderInvoiceDetail.context
        );
        this.dispatch('invoice:addressInfos', fetchAddressInfos.context);
