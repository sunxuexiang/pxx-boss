...
const importGoods = (ext) => {
  return Fetch('/goods/sort/import/' + ext, {
    method: 'GET'
  });
};
...
export default class GoodsImport extends React.Component<any, any> {
    ...
    return (
      <div>
        ...
        <div className="container">
          <Headline title="商品排序导入" />
          <Alert
            message="操作说明："
            description={
              <ul>
                <li>
                  1、请先下载商品排序导入模板，按照批注中的要求填写数据，未按要求填写将会导致导入失败。
                </li>
                <li>
                  2、请选择 .xlsx或
            ...
            <Spin spinning={this.state.loading}>
              <div className="steps-content" style={styles.center}>
                <Dragger
                  ...
                  accept=".xls,.xlsx"
                  headers={header}
                  action={Const.HOST + '/goods/sort/excel/upload'}
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref = Const.HOST + `/goods/sort/excel/template/${encrypted}`;
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/goods/sort/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
