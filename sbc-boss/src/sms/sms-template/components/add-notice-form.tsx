...
export default class AddNoticeForm extends React.Component<any, any> {
  constructor(props) {
    super(props);
    this.state = {
      templateName: '',
  ...
  componentWillReceiveProps() {
    const data = this.props.noticeFormData;
  ...
  render() {
    ...
    return (
      <Form className="sms-form">
        <FormItem label="通知节点" {...formItemLayout}>
          {getFieldDecorator('businessType', {
            initialValue: businessType,
            rules: [{ required: true, message: '请选择通知节点' }]
          })(
            <Select dropdownStyle={{ zIndex: 1053 }}>
              <Option value={null}>请选择</Option>
              {notices.toJS().map((item) => {
                return (
