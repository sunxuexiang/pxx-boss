  ...
  _handleSubmit = () => {
    ...
    form.validateFields(null, (errs) => {
      if (!errs) {
        ...
        //提交
        if (formData.get('couponCateName')) {
          doAdd();
        }
      } else {
        this.setState({});
...
class CateModalForm extends React.Component<any, any> {
  ...
  render() {
    ...
    const couponCateName = formData.get('couponCateName');
    const { getFieldDecorator } = this.props.form;

    return (
      <Form className="login-form">
        <FormItem {...formItemLayout} label="分类名称" hasFeedback>
          {getFieldDecorator('couponCateName', {
            rules: [
  ...
  _changeCateName = (e) => {
    ...
    editFormData(Map({ couponCateName: e.target.value }));
  };
}
