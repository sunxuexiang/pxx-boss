...
export default class AppStore extends Store {
  ...
  init = async (wareId, activityId, callback) => {
    if (activityId) {
      await this.editInit(wareId, activityId, callback);
    }
  };
   ...
   * 编辑初始化
   */
  editInit = async (wareId, advertisingId, callback) => {
    // 1.查询活动详情
    ...
    if (res.code == Const.SUCCESS_CODE) {
      ...
      activity.jumpLink.isSuit = homeacTive.advertisingConfigList[0].isSuit
        ...
        : 0;
      activity.dateNum = homeacTive.advertisingConfigList[0].dateNum || '';
      activity.activityTitle =
        homeacTive.advertisingConfigList[0].activityTitle || '';
      activity.dateType = homeacTive.advertisingConfigList[0].dateType || '1';
      ...
      if (activity.jumpLink.isSuit === 6) {
        callback(homeacTive.advertisingConfigList[0].advertisingVOList || []);
      }
    }
  };
   ...
   * 新增/编辑优惠券
   */
  save = async (selectStoreRows) => {
    // 1.从state中获取数据
    let activity = this.state().get('activity').toJS();
    let imageList = this.state().get('images').toJS();
      ...
      params.advertisingConfigList = [
        {
          ...
          moFangPageCode: activity.jumpLink.pageCode
            ...
            : '',
          dateNum: activity.jumpLink.isSuit == 5 ? activity.dateNum : '',
          dateType: activity.jumpLink.isSuit == 5 ? activity.dateType : ''
        }
      ];
      if (activity.jumpLink.isSuit === 6) {
        const advertisingIds = [];
        selectStoreRows.forEach((item) => {
          advertisingIds.push(item.advertisingId);
        });
        params.advertisingConfigList[0].advertisingIds =
          advertisingIds.join(',');
        params.advertisingConfigList[0].activityTitle = activity.activityTitle;
      }
