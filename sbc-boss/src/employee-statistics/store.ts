...
export default class AppStore extends Store {
  ...
  employeeStatistics = async (params) => {
    ...
    if (params instanceof Array) {
      ...
      dateType = this.state().get('dateType');
    }
    this.transaction(() => {
      //全局时间类型
    ...
    const sort = this.getAchieveSortName(
  ...
  onAchievePagination = async (pageNum, pageSize, sort) => {
    ...
    const { res } = await webapi.getAchieveTable({
      companyId: this.state().get('companyId'),
      dataCycle:
  ...
  setCompanyInfoId = (id: string) => {
    this.dispatch('employee:companyId', id);
    //数据刷新
    this.employeeStatistics(null);
  };
}
