...
const importGoods = (ext) => {
  return Fetch('/goods/brand/sort/import/' + ext, {
    method: 'GET'
  });
};
...
export default class GoodsImport extends React.Component<any, any> {
            ...
            <Spin spinning={this.state.loading}>
              <div className="steps-content" style={styles.center}>
                <Dragger
                  ...
                  accept=".xls,.xlsx"
                  headers={header}
                  action={Const.HOST + '/goods/brand/sort/excel/upload'}
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/goods/brand/sort/excel/template/${encrypted}`;
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/goods/brand/sort/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
