...
export default class RegisteredAddForm extends React.Component<any, any> {
  ...
  componentWillReceiveProps(nextpops) {
    ...
    this.setState({
      selectedRows: fromJS(actvie.selectedRows), selectedRows1: fromJS(actvie.selectedRows1),
  ...
  render() {
    ...
    const images1 = store.state().get('images1').toJS();
    const imageUrl = store.state().get('activity').get('imageUrl');
    const imageUrl1 = store.state().get('activity').get('imageUrl1');
    const images2 = store.state().get('images2').toJS();
    const imageUrl2 = store.state().get('activity').get('imageUrl2');
    const images3 = store.state().get('images3').toJS();
    const imageUrl3 = store.state().get('activity').get('imageUrl3');
    const images4 = store.state().get('images4').toJS();
    const imageUrl4 = store.state().get('activity').get('imageUrl4');

    const isSuit = activity.get('jumpLink').toJS().isSuit ? activity.get('jumpLink').toJS().isSuit : 0;
    const isSuit1 = activity.get('jumpLink1').toJS().isSuit1 ? activity.get('jumpLink1').toJS().isSuit1 : 0;
    const isSuit2 = activity.get('jumpLink2').toJS().isSuit2 ? activity.get('jumpLink2').toJS().isSuit2 : 0;
    const isSuit3 = activity.get('jumpLink3').toJS().isSuit3 ? activity.get('jumpLink3').toJS().isSuit3 : 0;
    const isSuit4 = activity.get('jumpLink4').toJS().isSuit4 ? activity.get('jumpLink4').toJS().isSuit4 : 0;
    const { getFieldDecorator } = form;
    const { skuExists, selectedRows, skuExists1, selectedRows1, statusa, selectedRows2, selectedRows3, selectedRows4 } = this.state;
