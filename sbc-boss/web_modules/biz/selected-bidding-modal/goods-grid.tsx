...
export default class GoodsGrid extends React.Component<any, any> {
  ...
  componentDidMount() {
    ...
    this.init({cateId:cateId});
  }

  componentWillReceiveProps(nextProps) {
    if (!this.props.visible && nextProps.visible) {
      this.init({cateId:nextProps.cateId});
    }
    this.setState({
  ...
  render() {
    ...
    const { rowChangeBackFun, visible, checkAddedGood,biddingType } = this.props;
    return (
  ...
  init = async (params) => {
    ...
    if ((res as any).code == Const.SUCCESS_CODE) {
      ...
      this.setState({
        ...
        loading: false,
        searchParams:{cateId:params.cateId}
      });
    }
  };
