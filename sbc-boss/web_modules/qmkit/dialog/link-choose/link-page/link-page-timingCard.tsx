...
import TimgingCardItem from "./timing-card-item";
...
export default class LinkPageProducts extends React.Component<any, any> {
            ...
            <div className="products_list">
              <ul>
                {goodList.map((item, index) => {
                  return (
                    <TimgingCardItem
                      key={item.timingCardId}
                      item={item}
                      active={this.state.chosenId === item.timingCardId}
  ...
  _getLinkInfo = () => {
    ...
    this.setState({ chosenId: chooseData.get("timingCardId") });
  };
  /**
   * 点击快速跳转某页面触发，重新调接口
   */
  ...
  _chosen = index => {
    this.setState({
      index,
      chosenId: this.state.goodList[index].timingCardId,
    });
  ...
  async _fetchGoodsList(type) {
    ...
    let url = "/xsite/timing-card-page";
    let data = await D2cLinkApi.getTimgCardList(api_host, url, type);
