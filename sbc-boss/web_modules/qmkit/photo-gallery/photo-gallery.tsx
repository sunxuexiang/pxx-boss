...
export default function PhotoGallery(props) {
  ...
  const checkUploadFile = (file, fileList) => {
    if (fileList.length > maxChoose) {
      if (fileList.filter((f) => f.uid).length == fileList.length) {
        message.error(`最多一次性上传${maxChoose}张图片`);
      }
      return false;
    }
  ...
  // 确认添加
  const confirmAddImg = () => {
    if (imgList.length + checkImgList.length > maxChoose) {
      message.warning(`当前最多可选择${maxChoose}张`);
      return;
    }
      ...
      >
        <div className="photo-gallery-modal">
          ...
          <div className="photo-gallery-modal-head">
            ...
            >
              <Button onClick={() => handleUploadClick()}>本地上传</Button>
            </QMUpload>
            {/* 文件名 */}
            <Input
              style={{ marginLeft: 20 }}
